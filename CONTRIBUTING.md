...
Check [open issues](https://github.com/Har1sh-k/CrossFire/issues) for ideas. Good first contributions:
...
Skills live in `crossfire/skills/`. Each skill:

1. Inherits from `BaseSkill` (`crossfire/skills/base.py`)
...
4. Is registered in `crossfire/core/orchestrator.py:_run_skills()`
5. Has a toggle in `crossfire/config/defaults.py` under `agents.skills`

---

## Adding a new agent

Agent adapters live in `crossfire/agents/`. Each adapter:

1. Inherits from `BaseAgent` (`crossfire/agents/base.py`)
2. Implements `_call_cli()` and `_call_api()` (or just one if the other isn't applicable)
3. Is registered in `crossfire/agents/review_engine.py` and `crossfire/agents/debate_engine.py`
4. Has a config section in `crossfire/config/defaults.py`
...
Open a [GitHub Discussion](https://github.com/Har1sh-k/CrossFire/discussions) or file an issue.
