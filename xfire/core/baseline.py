...
Reads/writes .xfire/baseline/ to enable:
...
from xfire.core.models import Finding, IntentProfile
...
class BaselineManager:
    """Manages the .xfire/baseline/ directory for a repository."""
    ...
    def _do_build(
        ...
        if base_ref:
            ...
            # No ref given (e.g. xfire baseline .) — use working tree
    ...
    def load(self) -> Baseline:
        """Load baseline from .xfire/baseline/."""
        if not self.exists():
            raise FileNotFoundError(
                f"No baseline found at {self.baseline_dir}. "
                "Run `xfire baseline .` first."
            )
    ...
    async def check_intent_changed(
        ...
        """
        from xfire.agents.prompts.context_prompt import check_intent_changed
...
def _collect_config_files_at_ref(repo_dir: str, ref: str) -> dict[str, str]:
    ...
    from xfire.core.context_builder import MANIFEST_FILES, SECURITY_CONFIG_PATTERNS
...
def _get_directory_structure_at_ref(repo_dir: str, ref: str) -> str:
    ...
    from xfire.core.context_builder import _get_directory_structure
