...
from xfire.agents.tools import GEMINI_TOOLS, MAX_TOOL_ITERATIONS, execute_tool
from xfire.auth import get_gemini_access_token
...
class GeminiAgent(BaseAgent):
    ...
    async def _run_api(
        ...
        if api_key:
            ...
            from xfire.auth.store import _is_expired, read_gemini_cli_credentials
        ...
        for iteration in range(MAX_TOOL_ITERATIONS):
            ...
            if response.status_code in (401, 403):
                raise AgentError(
                    self.name,
                    "Gemini token unauthorized — re-run `xfire auth login --provider gemini`.",
                )
