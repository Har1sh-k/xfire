...
def _apply_output_flags(
    ...
    if debug:
        from xfire.output.debug_log import DebugCollector
    ...
    for name, cfg in settings.agents.items():
        ...
        if cfg.mode == "cli":
            ...
            if key:
                ...
                results[name] = (True, "subscription auth configured (.xfire/auth.json)")
            else:
                results[name] = (
                    False,
                    f"missing credentials ({cfg.api_key_env} or xfire auth login --provider {name})",
                )
...
def auth_login(
    ...
    if provider_norm == "claude":
        ...
        from xfire.auth import read_claude_cli_credentials
...
def auth_status() -> None:
    ...
    console.print(table)
    console.print(f"[dim]Auth store: {auth_path}[/dim]")
    console.print("[dim]Run `xfire auth login --provider <name>` for setup help.[/dim]")
...
def analyze_pr(
    ...
    debug: bool = typer.Option(False, "--debug", help="Write full debug trace to xfire-debug-TIMESTAMP.md"),
    ...
    if _collector is not None:
        from xfire.output.debug_log import write_debug_markdown
...
def analyze_diff(
    ...
    \b
    Examples:
      xfire analyze-diff --staged
      xfire analyze-diff --patch changes.patch
    ...
    if commit:
        ...
        tmp = tempfile.NamedTemporaryFile(
            mode="w", suffix=".patch", prefix="xfire_",
...
def baseline(
    ...
    """Build persistent repo baseline context in .xfire/baseline/."""
    from xfire.config.settings import ConfigError, load_settings
    ...
    if mgr.exists() and not force:
        console.print(
            "[yellow]Baseline already exists at .xfire/baseline/[/yellow]\n"
    ...
    console.print(f"\nFiles written to: {repo_dir}/.xfire/baseline/")
...
def scan(
    ...
    from xfire.config.settings import ConfigError, load_settings
    from xfire.core.baseline import BaselineManager
    from xfire.core.diff_resolver import DiffResolver, DiffResolverError
    from xfire.core.orchestrator import CrossFireOrchestrator
...
def init() -> None:
    ...
    if config_file.exists():
        console.print("[yellow]Config already exists at .xfire/config.yaml[/yellow]")
        raise typer.Exit(0)

    example = Path(__file__).parent.parent / ".xfire" / "config.example.yaml"
    if example.exists():
        shutil.copy(example, config_file)
    else:
        config_file.write_text(_default_config_yaml(), encoding="utf-8")

    console.print("[green]Created .xfire/config.yaml[/green]")
...
def test_llm(
    ...
    \b
    Examples:
      xfire test-llm                          # test all agents in default mode
      xfire test-llm --agents claude          # test only Claude
      xfire test-llm --mode api               # force API mode for all
      xfire test-llm --thinking               # enable extended thinking
      xfire test-llm --prompt "What is 2+2?"  # custom prompt
    """
    import asyncio
    import time as _time
    ...
    from xfire.agents.review_engine import AGENT_CLASSES
    from xfire.config.settings import ConfigError, load_settings
    ...
    if not enabled:
        _handle_error("No agents are enabled. Check .xfire/config.yaml.")

    from xfire.cli_ui import AgentTestUI, render_banner, render_stats
...
def demo(
    ...
    UI demo (no LLM calls):
      xfire demo --ui                          # all 3 scenarios
      xfire demo --ui --scenario both_accept   # one scenario
